...
FIX Windows Python 3.12:
  - StackingClassifier n_jobs=1  → tránh nested multiprocessing crash (WinError 87)
  - HGBT không có n_jobs         → đã bỏ tham số
  - GBT không có n_jobs          → đã bỏ tham số
  - RF dùng n_jobs=14            → tận dụng 14 core vật lý
  - Env vars set TRƯỚC import    → joblib nhận đúng backend
...
def build_stacking(ratio) -> Pipeline:
    ...
    Cấu hình n_jobs đúng cho Windows + sklearn:
    ┌──────────────────────┬──────────┬──────────────────────────────────────┐
    │ Estimator            │ n_jobs   │ Lý do                                │
    ├──────────────────────┼──────────┼──────────────────────────────────────┤
    │ StackingClassifier   │    1     │ FIX: tránh WinError 87 (nested MP)   │
    │ HGBT                 │  ❌ N/A  │ sklearn không hỗ trợ n_jobs          │
    │ GBT                  │  ❌ N/A  │ sklearn không hỗ trợ n_jobs          │
    │ RF (trong stack)     │   14     │ dùng đủ core, bù cho HGBT/GBT       │
    └──────────────────────┴──────────┴──────────────────────────────────────┘
    """
    spw = min(ratio, 15.0)
